...
class Game extends Component {
  state = {
    categoryId: 'FRUIT',
    score: 0,
    randomImageDetails: imagesList[0],
  ...
  refreshActiveImage = () => {
    ...
    const randomImageDetailsId = Math.floor(Math.random() * imagesListLength)
    const randomImageDetails = imagesList[randomImageDetailsId]
    console.log(randomImageDetails)
    this.setState({randomImageDetails})
  }

  getActiveImage = activeImageId => {
    const {randomImageDetails} = this.state
    if (randomImageDetails.id === activeImageId) {
  ...
  onClickPlayAgainButton = () => {
    ...
    const randomImageDetails = imagesList[randomImageDetailsId]
    this.setState({randomImageDetails})
    this.setState({showScoreBoard: false, score: 0, timer: 60})
  }
  ...
  render() {
    ...
    const activeImageUrl = randomImageDetails.imageUrl
        ...
        {showScoreBoard ? (
          ...
          <div>
            <div className="image-container">
              <img className="big-image" alt="match" src={activeImageUrl} />
            </div>
            <ul className="tabs-container">
              {tabsList.map(eachItem => (
                <TabItems
                  isActive={eachItem.tabId === categoryId}
                  key={eachItem.id}
            ...
            <ul className="thumbnail-images-container">
              {filteredImagesList.map(eachItem => (
                <ImageItems
                  key={eachItem.id}
                  getActiveImage={this.getActiveImage}
                  imageItem={eachItem}
                />
              ))}
            </ul>
